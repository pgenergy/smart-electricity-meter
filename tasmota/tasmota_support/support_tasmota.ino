...
const uint16_t PROGMEM ENERGYLEAF_PORT = 443;
...
const char PROGMEM ENERGYLEAF_POST_AUTH[] = "POST /api/v1/token HTTP/1.1\r\n"
                                     ...
                                     const char PROGMEM ENERGYLEAF_POST_AUTH_TEST[] = "POST /api/v1/token HTTP/1.1\r\n"
...
bool ENERGYLEAF_X509A_LOADED = false;
BearSSL::WiFiClientSecure_light *wifiClientToken;
...
if(!ENERGYLEAF_ACTIVE) {
  ...
  if(WiFi.status() == WL_CONNECTED){
    ...
    if (pb_encode(&stream_out, TokenRequest_fields, &tokenrequest) && ENERGYLEAF_X509A_LOADED && wifiClientToken->connect(ENERGYLEAF_HOST,ENERGYLEAF_PORT)){//} && wifiClientToken.connect(ENERGYLEAF_HOST,ENERGYLEAF_PORT)) {
      
      AddLog(LOG_LEVEL_INFO,ENERGYLEAF_POST_AUTH_TEST);
      
      wifiClientToken->write("POST /api/v1/token HTTP/1.1\r\n");
      wifiClientToken->write("Host: ");
      wifiClientToken->write(ENERGYLEAF_HOST);
      wifiClientToken->write("\r\n");
      wifiClientToken->write("Content-Type: application/x-protobuf\r\n");
      wifiClientToken->write("Content-Length: ");
      wifiClientToken->print(stream_out.bytes_written);
      wifiClientToken->write("\r\n\r\n");
